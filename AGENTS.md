...
**Indus** is a full-stack financial intelligence platform built with Next.js 15 (App Router), React 19, TypeScript 5, and Supabase. Users authenticate, browse stocks and crypto, view 50+ financial metrics, stream real-time price data via SSE, interact with an agentic AI financial analyst (Gemini via Vercel AI SDK), and generate AI-powered research reports.
...
### AI Agent System
The AI chat uses Vercel AI SDK's `streamText()` with tool definitions. Gemini autonomously decides which tools to call (fetch stock prices, get financial metrics, compare companies) and synthesizes results. Model routing: Gemini 2.5 Flash for quick explanations, Gemini 2.5 Pro for multi-turn chat and report generation.
...
| `GOOGLE_GENERATIVE_AI_API_KEY` | Server-only | Gemini API key (used via Vercel AI SDK) |
...
### Code Style
- Biome handles linting and formatting. Run `bun check` before committing.
...
### Testing
- Unit/integration tests: Vitest. Run with `bun test`.
- E2E tests: Playwright. Run with `bun test:e2e`.
