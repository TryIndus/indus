...
The primary React application uses one Chromium full-stack suite. Run it with `cd apps/web && bun run test:e2e`. The command starts disposable PostgreSQL and Rails services, builds the Vite application, and covers the public landing and sign-in lifecycle, durable favorite and report mutations, report cancellation, stock and slash-delimited crypto search, a model-assisted brief, unauthenticated and cross-tenant boundaries, recovery behavior, responsive navigation, accessibility, and the local shell-load budget. Only external identity, market, and model providers are substituted behind a Rails test-environment guard; the project-scoped containers and storage are removed on exit.
