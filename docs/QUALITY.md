...
The primary React application uses one Chromium full-stack suite. Run it with `cd apps/web && bun run test:e2e`. The command starts disposable PostgreSQL and Rails services, builds the Vite application, and covers public and authenticated journeys, a durable mutation, the authorization boundary, recovery behavior, responsive navigation, accessibility, and the local shell-load budget. Only external identity and market providers are substituted behind a Rails test-environment guard; the project-scoped containers and storage are removed on exit.
