...
resource "aws_cloudfront_distribution" "this" {
  ...
  default_cache_behavior {
    ...
    allowed_methods = var.edge_runtime == "legacy-next" ? [
      "DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "POST", "PUT"
    ] : ["GET", "HEAD", "OPTIONS"]
    cached_methods = ["GET", "HEAD", "OPTIONS"]
    compress       = true
    cache_policy_id = var.edge_runtime == "legacy-next" ? (
      data.aws_cloudfront_cache_policy.disabled.id
    ) : data.aws_cloudfront_cache_policy.optimized.id
    origin_request_policy_id = var.edge_runtime == "legacy-next" ? (
      data.aws_cloudfront_origin_request_policy.all_except_host.id
    ) : null
  }
