...
- `src/lib/market-stream.ts` defines the live-market interface. The initial implementation deliberately opens no provider connection. The crypto view uses bounded instrument discovery and fundamentals while presenting live-stream availability explicitly.
