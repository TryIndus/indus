...
export function AuthPage() {
  ...
  const submit = (event: FormEvent) => {
    ...
    void perform(async () => {
      if (mode === 'signin') { if (!auth.passwordSignIn) return auth.signIn(); await auth.passwordSignIn(email, password); await navigate({ to: '/dashboard', replace: true }); return }
      if (mode === 'signup') { if (!auth.signUp) throw new Error('Account creation is unavailable.'); const result = await auth.signUp(email, password, firstName, lastName); if (result === 'confirmation-required') { setMode('confirm'); setMessage('Enter the verification code sent to your email.') } else setMode('signin'); return }
      if (mode === 'confirm') { if (!auth.confirmSignUp) throw new Error('Account confirmation is unavailable.'); await auth.confirmSignUp(email, code); setMode('signin'); setMessage('Account confirmed. You can now sign in.'); return }
      if (mode === 'forgot') { if (!auth.requestPasswordReset) throw new Error('Password recovery is unavailable.'); await auth.requestPasswordReset(email); setMode('reset'); setMessage('Enter the recovery code sent to your email.'); return }
      if (!auth.confirmPasswordReset) throw new Error('Password recovery is unavailable.'); await auth.confirmPasswordReset(email, code, password); setMode('signin'); setMessage('Password updated. You can now sign in.')
    })
  }
  ...
  return <div className="auth-layout">
    ...
    <main className="auth-main"><div className="auth-form-wrap"><div className="mobile-auth-brand"><a href="/" className="brand-link"><img src="/logo.svg" alt="" />Indus</a><a href="/"><ArrowLeft />Home</a></div><p className="auth-eyebrow">{eyebrow}</p><h2>{heading}</h2><p className="auth-intro">{mode === 'signup' ? 'Create an account to save companies and generated research.' : mode === 'signin' ? 'Sign in to return to your watchlist, reports, and company analysis.' : 'Use the email associated with your Indus account.'}</p>
      {mode === 'signin' && <><button type="button" className="oauth-button" disabled={busy} onClick={() => void perform(async () => { await auth.signIn(); if (await auth.getUser()) await navigate({ to: '/dashboard', replace: true }) })}>Continue with Google or SSO</button><div className="auth-divider"><span/>Or use email<span/></div></>}
