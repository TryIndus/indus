...
function BrandMark({ className = "size-6" }: { className?: string }) { return <img src="/logo.svg" alt="" className={className} /> }
function Button({ asChild, variant = "primary", size, className = "", children, ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { asChild?: boolean; variant?: string; size?: string; children: ReactNode }) {
 const classes = `ui-button ${variant === "outline" ? "ui-button-outline" : variant === "ghost" ? "ui-button-ghost" : ""} ${size === "lg" ? "ui-button-lg" : ""} ${className}`
 if (asChild && isValidElement(children)) return cloneElement(children as ReactElement<{ className?: string }>, { className: `${classes} ${(children.props as { className?: string }).className ?? ""}` })
 return <button className={classes} {...props}>{children}</button>
}
function BackToTopButton() { return <a href="#top" className="back-to-top" aria-label="Back to top">↑</a> }
